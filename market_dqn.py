...
if __name__ == "__main__":
    ...
    env = MarketEnv(dir_path = "./sample_data/", target_codes = codeMap.keys(), input_codes = [], \
                    start_date = "2013-08-26", end_date = "2015-08-25", sudden_death = -1.0)
    ...
    for e in range(epoch):
        ...
        while not game_over:
            ...
            if env.actions[action] == "LONG" or env.actions[action] == "SHORT":
                ...
                print("#%i ->  %s:\t%s\t%.2f\t%.2f\t" % (e, info["dt"], color + env.actions[action] + bColors.ENDC, cumReward, info["cum"]) + ("\t".join(["%s:%.2f" % (l, i) for l, i in zip(env.actions, q[0].tolist())]) if isRandom == False else ""))
            ...
            present_state = flatten(input_tm1)
            next_state = flatten(input_t)

            line = "{\"correlationId\": \"%i\", \"state\": %s, \"action\": \"%s\", \"reward\": %f, \"nextState\": %s,\"timeStamp\": %i, \"terminalState\": %s}\n" \
                   % (e, present_state, env.actions[action], reward, next_state ,  int(round(time.time() * 1000)), str(game_over).lower())
            tr_file.write(line)
